    ...
    <div class="std-curd">
        ...
        >
            <std-data-entry ref="std_data_entry" :data-list="editableColumns()" :data-source="data"
                            :error="error" @uploaded="get_list()">
                <div slot="supplement">
                    <slot name="supplement" :data="data"/>
                </div>
                <div slot="action">
        ...
        <footer-tool-bar v-if="batch_columns.length">
            <a-space>
                ...
                <a-button
                    v-for="c in batch_columns"
                    :key="c.dataIndex"
                    type="primary"
                    :disabled="!selected.length"
                    @click="visible_batch_edit=true;current_batch_columns=[c]"
                    ghost>
                    修改{{ c.title }}
                </a-button>
            </a-space>
        ...
        >
            留空则不修改
            <std-data-entry :data-list="current_batch_columns" :data-source="data"/>
        </a-modal>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        okBatchEdit() {
            this.api.batchSave(this.selected, this.data).then(() => {
        ...
        editableColumns() {
            return this.columns.filter((c) => {
                if (c.beforeSend) {
                    this.beforeDataSendFn[c.dataIndex] = c.beforeSend
                }
                return c.edit
            })
        },
        ...
        async ok() {
            this.error = {}
            for (const [dataIndex, fn] of Object.entries(this.beforeDataSendFn)) {
                await fn(this.data[dataIndex], this.data)
            }
            if (this.data.id) {
